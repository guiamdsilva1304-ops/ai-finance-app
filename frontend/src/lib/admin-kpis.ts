...
// Cálculo dos KPIs do admin — compartilhado entre /api/admin/kpis e o
// assistente IA do admin (que injeta isso no system prompt).

export const PRECO_PRO = 14.9;
export const PRECO_PREMIUM = 39.9;
...
export async function computeKpis(supabase: SupabaseClient): Promise<Kpis> {
  ...
  const msgsAtual = mediaMsgs(msgs7.data as { user_id: string }[] | null);
  const msgsAnterior = mediaMsgs(msgs14.data as { user_id: string }[] | null);
