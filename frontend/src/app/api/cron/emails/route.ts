...
export async function GET(req: NextRequest) {
  ...
  if (req.nextUrl?.searchParams.get("debug") === "1") return NextResponse.json({ secret, env: process.env.imoneycronsecret2026 ?? "UNDEFINED", all_keys: Object.keys(process.env).filter(k => k.includes("CRON")) });
