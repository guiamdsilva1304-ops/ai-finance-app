...
export default function AuthPage() {
  ...
  function parseErr(err: { message: string; status?: number }): string {
    ...
    if (msg.includes("password should be")) return "Senha muito curta. Use mínimo 6 caracteres.";
    ...
    if (msg.includes("api key") || msg.includes("apikey")) return "Erro de configuração. Contate o suporte.";
  ...
  async function handleLogin(e: React.FormEvent) {
    ...
    try {
      ...
      setError(e instanceof Error ? `Erro: ${e.message}` : "Erro inesperado.");
    ...
    <div className="min-h-screen bg-[#f8fdf9] bg-dots flex flex-col">
      ...
      <main className="flex-1 flex items-start justify-center px-4 pb-16">
        <div className="w-full max-w-md animate-fade-up">
          <div className="bg-white rounded-2xl border border-[#e4f5e9] overflow-hidden"
            style={{ boxShadow: "0 8px 40px rgba(20,83,45,0.12)" }}>
            {/* Tabs */}
            <div className="flex border-b border-[#e4f5e9]">
              {(["login", "register"] as Tab[]).map(t => (
                <button key={t} onClick={() => { setTab(t); setError(""); setSuccess(""); }}
                  className={cn("flex-1 py-4 text-sm font-bold transition-all",
                    tab === t ? "text-[#15803d] border-b-2 border-[#16a34a] bg-[#f0fdf4]"
                              : "text-[#8db89d] hover:text-[#15803d]"
              ...
              {tab === "login" ? (
                <form onSubmit={handleLogin} className="space-y-5">
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <input type="email" value={email} onChange={e => setEmail(e.target.value)}
                        ...
                        className="input pl-10" disabled={loading}/>
                    </div>
                  </div>
                  <div>
                  ...
                  <button type="submit" disabled={loading} className="btn-primary w-full mt-2">
                    {loading
                      ? <span className="flex items-center gap-2"><span className="h-4 w-4 rounded-full border-2 border-white/30 border-t-white animate-spin"/>Entrando...</span>
                ...
                <form onSubmit={handleRegister} className="space-y-5">
                        ...
                        <div className="flex gap-1 flex-1">
                          {[1,2,3].map(i => (
                            <div key={i} className={cn("h-1.5 flex-1 rounded-full transition-all",
                              i <= passStrength
                                ? ["bg-red-400","bg-amber-400","bg-[#4ade80]"][passStrength-1]
                                : "bg-gray-200")}/>
                          ))}
                        </div>
                        <span className="text-[11px] font-bold text-[#6b9e80]">
                          {["Fraca","Média","Forte"][passStrength-1] ?? ""}
                        </span>
                      </div>
                    )}
                  </div>
                  ...
                  <button type="submit" disabled={loading} className="btn-primary w-full mt-2">
                    {loading
                      ? <span className="flex items-center gap-2"><span className="h-4 w-4 rounded-full border-2 border-white/30 border-t-white animate-spin"/>Criando conta...</span>
          ...
          <div className="flex items-center justify-center gap-6 mt-6">
            {[{icon:"🔒",text:"Dados criptografados"},{icon:"🏦",text:"Open Finance BCB"},{icon:"🤖",text:"IA com Claude"}].map(({icon,text}) => (
