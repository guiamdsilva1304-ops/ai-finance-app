...
export default function OpenFinancePage() {
  ...
  async function register() {
    ...
    setLoading(false);
    if (err) { setError("Erro ao registrar. Tente novamente."); return; }
    setDone(true);
    ...
    <div className="p-5 lg:p-8 max-w-3xl mx-auto">
      ...
      <div className="rounded-3xl bg-gradient-to-br from-[#0d2414] to-[#16a34a] p-8 text-white mb-6 relative overflow-hidden">
        ...
        <div className="relative">
          ...
          <h2 className="text-2xl font-black mb-3" style={{ fontFamily: "Nunito, sans-serif" }}>
            Seus extratos bancários direto no iMoney
          </h2>
          <p className="text-green-200 text-sm leading-relaxed mb-0">
            Estamos desenvolvendo a integração com Open Finance para você conectar seus bancos e ter todas as transações importadas automaticamente — sem digitar nada.
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
        {[
          ...
          { icon: <Shield size={20}/>, title: "100% seguro", desc: "Regulamentado pelo Banco Central. Seus dados protegidos pela LGPD" },
        ].map(({ icon, title, desc }) => (
          <div key={title} className="card text-center p-5">
            <div className="w-10 h-10 rounded-xl bg-[#f0fdf4] text-[#16a34a] flex items-center justify-center mx-auto mb-3">
              {icon}
            </div>
            <p className="font-black text-[#0d2414] text-sm mb-1" style={{ fontFamily: "Nunito, sans-serif" }}>{title}</p>
      ...
      <div className="card border-[#bbf7d0] bg-[#f8fdf9]">
        {done || alreadyRegistered ? (
          <div className="text-center py-6">
            ...
            <p className="font-black text-[#0d2414] text-lg mb-2" style={{ fontFamily: "Nunito, sans-serif" }}>
              {done ? "Você entrou na lista! 🎉" : "Você já está na lista! ✅"}
            </p>
            <p className="text-sm text-[#6b9e80] max-w-sm mx-auto">
              Te avisaremos assim que o Open Finance estiver disponível no iMoney. Fique de olho no seu e-mail!
            </p>
          </div>
        ) : (
          <>
            <div className="flex items-start gap-3 mb-5">
              ...
              <div>
                <p className="font-black text-[#0d2414]" style={{ fontFamily: "Nunito, sans-serif" }}>
                  Entre na lista de espera
                </p>
                <p className="text-sm text-[#6b9e80] mt-0.5">
                  Seja um dos primeiros a usar quando lançarmos. Sem spam, prometemos! 🤝
                </p>
              </div>
            </div>
            ...
            <p className="text-[11px] text-[#8db89d] mt-3 text-center">
              Já temos usuários na lista de espera · Gratuito para sempre para os primeiros
            </p>
          </>
        )}
      </div>
...
ENDOFFILE[
cat > /workspaces/ai-finance-app/frontend/src/app/dashboard/openfinance/page.tsx << 'ENDOFFILE'
"use client";
import { useEffect, useState } from "react";
import { createSupabaseBrowser } from "@/lib/supabase";
import { CheckCircle2, Landmark, Bell, Shield, Zap, ArrowRight } from "lucide-react";
