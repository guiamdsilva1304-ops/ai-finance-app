...
export default function OpenFinancePage() {
  ...
  async function register() {
    ...
    setLoading(true);
    const { data: { user } } = await supabase.auth.getUser();
    const { error: err } = await supabase.from("openfinance_interest").insert({ user_id: user?.id ?? null, email: email.trim() });
    setLoading(false);
      ...
      <div className="card border-[#bbf7d0] bg-[#f8fdf9]">
        {done || alreadyRegistered ? (
          <div className="text-center py-6">
            ...
            <p className="font-black text-[#0d2414] text-lg mb-2">{done ? "Você entrou na lista! 🎉" : "Você já está na lista! ✅"}</p>
            <p className="text-sm text-[#6b9e80] max-w-sm mx-auto">Te avisaremos assim que o Open Finance estiver disponível no iMoney.</p>
          </div>
        ) : (
          <>
            ...
            <div className="flex gap-2">
              <input type="email" value={email} onChange={e => setEmail(e.target.value)} placeholder="seu@email.com" className="input flex-1"/>
              <button onClick={register} disabled={loading} className="btn-primary shrink-0 flex items-center gap-2">
                {loading ? <span className="h-4 w-4 rounded-full border-2 border-white/30 border-t-white animate-spin"/> : <><span>Quero!</span><ArrowRight size={15}/></>}
              </button>
            </div>
