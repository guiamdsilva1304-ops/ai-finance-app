...
Para outros pedidos responda em markdown. Nunca mencione o app diretamente nos primeiros posts.`,
    sugestoes: [
...
{"artigo":{"titulo":"...","slug":"url-do-artigo","meta_description":"...","conteudo":"artigo completo em markdown","publicar_automaticamente":true}}

Use publicar_automaticamente: true para artigos informativos (70% dos casos). False para artigos com preços ou dados sensíveis.
...
{"acoes":[{"tipo":"email","descricao":"...","status":"executado","detalhe":"assunto e corpo"},{"tipo":"alerta","descricao":"...","status":"pendente","detalhe":"requer aprovação"}]}
...
Para arquitetura, análise ou discussão técnica responda em markdown normal.`,
    sugestoes: [
    ...
    <div style={{ background: '#fff', border: `1px solid ${cor}44`, borderRadius: 12, overflow: 'hidden', marginBottom: 8 }}>
      ...
      {exp && (
        <div style={{ padding: '12px 14px', display: 'flex', flexDirection: 'column', gap: 12 }}>
          ...
          {card.titulo && <div><div style={{ fontSize: 10, fontWeight: 700, color: '#aaa', letterSpacing: '0.06em', marginBottom: 4 }}>TÍTULO</div><div style={{ fontSize: 13, fontWeight: 600, color: '#1a1a1a', borderLeft: `3px solid ${cor}`, paddingLeft: 10 }}>{card.titulo}</div></div>}
  ...
  return (
    <div style={{ background: '#fff', border: '1px solid #D85A3033', borderRadius: 12, overflow: 'hidden', marginBottom: 8 }}>
      <div style={{ background: '#D85A3010', padding: '10px 14px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div><div style={{ fontSize: 10, fontWeight: 700, color: '#D85A30', marginBottom: 2, letterSpacing: '0.05em' }}>PATCH</div><div style={{ fontSize: 12, fontWeight: 600, color: '#1a1a1a' }}>{block.arquivo}</div></div>
        <button onClick={() => { navigator.clipboard.writeText(cmd); setCopiado(true); setTimeout(() => setCopiado(false), 2000) }}
...
function CardAction({ action }: { action: AgentAction }) {
  ...
  return (
    <div style={{ background: '#fff', border: `1px solid ${cor}33`, borderRadius: 10, padding: '10px 14px', marginBottom: 6, display: 'flex', gap: 12, alignItems: 'flex-start' }}>
      <div style={{ width: 36, height: 36, borderRadius: 8, background: cor + '18', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 8, fontWeight: 700, color: cor, flexShrink: 0, letterSpacing: '0.04em' }}>{icones[action.tipo] || action.tipo.toUpperCase()}</div>
      <div style={{ flex: 1 }}>
...
export default function AgentesPage() {
  ...
  const carregarHistorico = useCallback(async (agente: Agent) => {
    ...
    try {
      ...
      const msgs_raw: { role: string; content: string }[] = data.messages ?? []
      if (msgs_raw.length === 0) return
      const parsed: Message[] = msgs_raw.map(m => {
      ...
      setMemoriaCount(prev => ({ ...prev, [agente.id]: msgs_raw.length }))
    } catch { /* sem memória */ } finally { setCarregandoMemoria(false) }
  }, [mensagens])
  ...
  async function enviar(texto?: string) {
    ...
    try {
      ...
      const novaMsg: Message = { role: 'assistant', content: parsed.texto, ts: new Date(), cards: parsed.cards, diff: parsed.diff, actions: parsed.actions }
      setMensagens(prev => ({ ...prev, [agenteSelecionado.id]: [...historico, novaMsg] }))
