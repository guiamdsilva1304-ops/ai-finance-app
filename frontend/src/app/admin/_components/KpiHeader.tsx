...
function Delta({ valor, invertido, sufixo = "" }: { valor: number | null; invertido?: boolean; sufixo?: string }) {
  if (valor === null || Math.abs(valor) < 0.005) {
    return <span className="text-[10px] font-bold text-[#3a6b45]">— sem variação 7d</span>;
  }
  const positivoBom = invertido ? valor < 0 : valor > 0;
  return (
    <span className={`text-[10px] font-extrabold ${positivoBom ? "text-[#00C853]" : "text-[#ff5252]"}`}>
...
function Card({ titulo, valor, sub, delta }: { titulo: string; valor: string; sub?: string; delta?: React.ReactNode }) {
  return (
    <div className="rounded-2xl border border-[#00C853]/10 bg-[#0e1a10] p-4">
      <p className="text-[11px] font-bold uppercase tracking-wider text-[#3a6b45]">{titulo}</p>
      <p className="mt-1.5 text-2xl font-black text-white">{valor}</p>
      <div className="mt-1 flex items-center justify-between gap-2">
        {sub ? <span className="text-[11px] text-[#dff0e3]/50">{sub}</span> : <span />}
        {delta}
      </div>
    </div>
  );
...
export default function KpiHeader() {
  ...
  if (erro) return <p className="rounded-xl border border-[#ff5252]/30 bg-[#ff5252]/5 p-3 text-xs text-[#ff5252]">KPIs indisponíveis: {erro}</p>;

  if (!kpis) {
    return (
      <div className="grid grid-cols-2 gap-3 md:grid-cols-3">
        {Array.from({ length: 6 }).map((_, i) => (
          <div key={i} className="h-[104px] animate-pulse rounded-2xl bg-[#0e1a10]" />
        ))}
      </div>
    );
  }
